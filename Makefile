...
TESTS=$(shell find tests -type f -name '*.cpp' 2>/dev/null | sed -re 's|^(.+)cstest.cpp$$||;s|^(.+)\.cpp$$|\1|')
...
%.x: tests/cstest.dbg.o $(DBGOBJECTS)
	$(CXX) -o $@ $< tests/cstest.dbg.o $(DBGOBJECTS) $(CXXFLAGS) $(DBGFLAGS) $(LDFLAGS)
...
.PHONY: clean
clean:
	rm -f *.o $(NAME) main.c.* vgcore.* $(TESTOBJECTS) $(DBGOBJECTS) $(OBJECTS) $(TESTEXECS)
	rm -rf docs

.PHONY: linecount
linecount:
	wc -l Makefile README.md $(foreach dir,$(DIRECTORIES),$(dir)/*.hpp $(dir)/*.cpp) $(foreach test,$(TESTS),$(test).cpp) tests/cstest.hpp tests/tests/cstest.cpp 2>/dev/null | sort -k2,2
